...
It's [piping operator](https://elixir-lang.org/getting-started/enumerables-and-streams.html#the-pipe-operator) is sexy as hell and makes the functional programming aspect a lot more palatable.
...
Comparable to separating access to your projects via VHosts, each app under an umbrella project is running under a different port, yet retain their capability to share code and functionality through their respective namespaces.
...
That of course makes it a big deciding factor in whether this makes sense for your project at all: is it big enough to allow for sensible splitting without succumbing to the problem of an over engineered mess?
...
Writing this as a separate app also makes it easier to re-use it for other projects with similar requirements for its user data handling.
...
Authentication is the first and only part so far that requires cross-app access to code.

### Frontend

The main part of the application and the one that relies the least on the backend, as it's mostly static content.
All of the javascript base goes here, along with the generators and CSS files.
...
The separation of apps by default is in a backend app, handling all business logic and connection to a datastore, and a frontend app, handling all views and templates.
Since our main application doesn't rely on any backend, we're not going to use this one for now.
We do however need to add at least two applications to our project: sealas_api and sealas_sso.
